...
EbErrorType Y4MVideoSource::parse_file_info() {
    ...
    do {
        ...
        case 'C':  // Color space
        {
            ...
            } else if (strncmp("420p10", line, 6) == 0) {
                bit_depth_ = 10;
...
uint32_t Y4MVideoSource::read_input_frame() {
    ...
    const int bottom_padding = height_with_padding_ - height_;
    const int righ_padding = width_with_padding_ - width_;
    unsigned int read_len = 0;
    unsigned int i;
    // Y
    eb_input_ptr = frame_buffer_->luma;
    for (i = 0; i < height_; ++i) {
        read_len = (uint32_t)fread(eb_input_ptr, 1, width_, file_handle_);
        if (read_len != width_)
            return 0;  // read file error.

        memset(eb_input_ptr + width_, 0x14, righ_padding);
        eb_input_ptr += frame_buffer_->yStride;
        filled_len += frame_buffer_->yStride;
    }
    for (i = 0; i < bottom_padding; ++i) {
        memset(eb_input_ptr, 0x14, width_with_padding_);
        eb_input_ptr += frame_buffer_->yStride;
        filled_len += frame_buffer_->yStride;
    }
    // Cb
    ...
    for (i = 0; i < (height_ >> height_downsize); ++i) {
        read_len = (uint32_t)fread(
            eb_input_ptr, 1, (width_ >> width_downsize), file_handle_);
        if (read_len != width_ >> width_downsize)
            return 0;  // read file error.

        memset(eb_input_ptr + (width_ >> width_downsize),
               0x14,
               righ_padding >> width_downsize);
        eb_input_ptr += frame_buffer_->cbStride;
        filled_len += frame_buffer_->cbStride;
    }
    for (i = 0; i < (bottom_padding >> height_downsize); ++i) {
        memset(eb_input_ptr, 0x14, (width_with_padding_ >> width_downsize));
        eb_input_ptr += frame_buffer_->cbStride;
        filled_len += frame_buffer_->cbStride;
    }
